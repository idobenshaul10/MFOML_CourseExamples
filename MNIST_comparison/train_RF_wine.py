...
def train_tree_model(x, y, mode='classification',
	...
	model = WaveletsForestRegressor(mode=mode, trees=trees, depth=depth,
									seed=state, norms_normalization=nnormalization, vi_threshold=0.8)
	model.fit(x, y)
	return model
...
def main(seed, trees):
	try:
		wandb.init(project='MNIST_trial_experiments', entity='ibenshaul', mode="disabled",
				   tags=["wine_dataset"], reinit=True)
		wandb.config.update({'seed': seed})
	except:
		print("wandb not initialized right")

	X_train, Y_train, X_test, Y_test, feature_columns = create_dataset(seed)
	...
	rf = predict_WF_RF(rf_model, X_train, Y_train, X_test, Y_test)
