...
def train_tree_model(x, y, mode='classification',
					 ...
					 state=2000, nnormalization='samples') -> WaveletsForestRegressor:
	print('Training tree model...')
	wandb.config.update({'trees': trees, 'depth': depth, 'state': state, 'mode': mode, 'nnormalization': nnormalization})
...
def predict_WF_RF(rf_model, X, y, X_test, y_test):
	...
	for num_wavelets in tqdm(range(1, total_num_wavelets, total_num_wavelets // 50)):
...
def main(seed, trees, noise_level):
	np.random.seed(seed)
	try:
		wandb.init(project='Shai_Denoiser_trial_experiments', entity='<username>', mode="online",
	...
	rf_model = train_tree_model(X_train, Y_train, mode='regression', trees=trees, depth=8)
...
def create_dataset(noise_level=0.1):
	...
	# img = cv2.resize(img, 128, interpolation=cv2.INTER_AREA)
...
if __name__ == '__main__':
	noises = [0.3, 0.5, 0.6, 0.7, 0.8, 0.9]
