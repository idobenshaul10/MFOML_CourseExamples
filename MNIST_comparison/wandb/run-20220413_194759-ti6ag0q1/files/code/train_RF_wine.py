...
def predict_WF_RF(rf_model, X, y, X_test, y_test):
	...
	train_acc = (X_pred.squeeze() == y).sum() / X_pred.shape[0]
	print(f"TOTAL train_acc:{train_acc}")
	total_num_wavelets = len(rf_model.sorted_norms)

	for num_wavelets in tqdm(range(1, total_num_wavelets+1, 10)):
		...
		test_acc = (test_pred.squeeze() == y_test).sum() / test_pred.shape[0]
		print(f"test_acc {num_wavelets} wavelets:{test_acc}")

		result = {'num_wavelets':num_wavelets, 'WF_RF_train_acc':train_acc, 'WF_RF_test_acc': test_acc}
		print("***********************")
		try:
			wandb.log(result)
		except:
...
def main():
	...
	rf_model = train_tree_model(X_train, Y_train, mode='regression', trees=1, depth=50)
...
def create_dataset():
	...
	train_red, test_red = train_test_split(red_wine, test_size=0.1, random_state=4)
	train_white, test_white = train_test_split(white_wine, test_size=0.1, random_state=4)
