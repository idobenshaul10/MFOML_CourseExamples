...
class WaveletsForestRegressor:
	...
	def fit(self, X_raw, y):
		...
		# ************************NEW********************
		#
		# X_raw = (X_raw - np.min(X_raw, 0)) / ((np.max(X_raw, 0) - np.min(X_raw, 0)) + 1e-6)
		# self.X, self.y = X_raw, y
		...
		if type(X_raw) == np.ndarray:
			try:
				...
				self.min_max_scaler.fit(X_raw)
				X = self.min_max_scaler.transform(X_raw)
			except:
	...
	def predict(self, X, m=-1, start_m=0, paths=None):
		'''
