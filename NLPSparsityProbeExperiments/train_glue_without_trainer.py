...
def parse_args():
	...
	parser.add_argument("--use_norms", action="store_true",
						help="If use_norm, compute index for each layer, instead of "
							 "a fixed alpha for all layers(just computing the norm)")
...
def main():
	...
	wandb.init(project='SparsityForCourse', entity='ibenshaul', mode="online", tags=["NLP"])
	...
	analyzer = SparsityAnalyzer(layers=layers, train_loader=train_dataloader_analysis, use_index=not args.use_norms)
